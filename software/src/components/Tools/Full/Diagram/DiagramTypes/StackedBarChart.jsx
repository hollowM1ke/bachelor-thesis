...
const StackedBarChart = ({
    ...
    useLayoutEffect(() => {
        ...
        data.forEach(row => {
            const newYMin = (d3.sum(row.filter(e => e < 0)));
            const newYMax = (d3.sum(row.filter(e => e >= 0)));
        ...
        // scale to get position of row on the x-axis
        ...
        const preparedData = data.flatMap((e1, e1Idx) => {
            let posYSum = 0;
            let negYSum = 0;
            let yPos;
            let barHeight;
        ...
        svg.append('g')
            ...
            .data(preparedData)
            .enter().append('rect')
            .attr('x', function (d) { return x0((xAxisCategories && xAxisCategories.length > 0) ? xAxisCategories[d.row] : d.row); })
