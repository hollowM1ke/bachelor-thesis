...
const ScatterPlotChart = ({
    ...
    useLayoutEffect(() => {
        ...
        // scatter plot legend
        const scatterLegend = svg.selectAll('.lineLegend').data(headers)
            .enter().append('g')
        ...
        scatterLegend.append('text').text(function (d) { return d; })
        ...
        scatterLegend.attr('transform', function (d, i) {
            return 'translate(' + (width - legendLabelMaxLen) + ',' + (-30 - (i * 20)) + ')';
        });

        // generate 2d array with scatter points
        const scatterPoints = [];
        yVals.forEach((col, colIdx) => {
            ...
            scatterPoints.push(colPoints);
        });

        // Highlight the specie that is hovered
        const highlight = function (colIdx) {
            d3.selectAll('.dot')
                .transition()
                .duration(200)
        ...
        const doNotHighlight = function () {
            scatterPoints.forEach((col, colIdx) => {
                d3.selectAll(`.column${colIdx}`)
                    .transition()
                    .duration(200)
                    .style('fill', colorScale(colIdx))
        ...
        scatterPoints.forEach((col, colIdx) => {
            svg.selectAll('.circle')
                ...
                .attr('class', d => `dot column${colIdx}`)
                .on('mouseover', () => highlight(colIdx))
        ...
        // svg.selectAll('circle').filter((d, i) => (d.y === undefined || d.y === '')).remove();
    }, [
